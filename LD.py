...
class LD():
    def __init__(self, lamda, material, model='LD'):
        ...
        if self.material == 'Ag':
            ...
            Gamma = [0.053, 0.241, 0.345, 0.870, 2.494, 2.214]
            omega = [0.000, 0.415, 0.830, 2.969, 4.304, 13.32]
        elif self.material == 'Polysterene':
            n=1.57
            A=1.56
            B=0.00483
            C=0.00071305
        ...
        if self.model == 'Con':
             epsilon_D = np.zeros(len(self.lamda), dtype=complex)
             for i, w in enumerate(self.lamda):
                epsilon_D[i] = n**2
             self.epsilon = epsilon_D
        
        elif self.model == 'Cauchy':
             epsilon_D = np.zeros(len(self.lamda), dtype=complex)
             for i, w in enumerate(self.lamda):
                w=w*1e6
                n=(A+B/w**2+C/w**4)
...
if __name__ == '__main__':
    ...
    silver = LD(lamda, material='TiO2', model='Cauchy')
